...
public class DBSCAN {
    ...
    private static int dbscan(List<STPoint> datalist, double eps, int minPts) throws Exception {
        ...
        for (STPoint point : datalist) //STPoint := SetOfPoints.get(i);
            if(point.isUnClassified()) //IF STPoint.CiId = UNCLASSIFIED THEN
                if(expandCluster(datalist, point, cluster_Id, eps, minPts))
                    //IF ExpandCluster(SetOfPoints, STPoint, ClusterId, Eps, MinPts) THEN
    ...
    private static boolean expandCluster(List<STPoint> datalist,
        ...
        List<STPoint> seeds = regionQuery(datalist, point, eps); //seeds : =SetOfPoints. regionQuery (STPoint, Eps )

        if(seeds.size() < minPts) //IF seeds.size<MinPts THEN // no core point
        {

            chg_clu_id(point, -1); //SetOfPoint. changeCl Id (STPoint, NOISE)
            return false;
        }
...
/*        for (STPoint dataTuple : datalist) {
